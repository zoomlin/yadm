...
function PCMD() {
    if [[ "${PROMPT_MODE}" == 0 ]]; then
        # echo "%{$fg[violet]%}${vim_mode}$(PR_DIR) $(PR_ERROR)$(PR_ARROW) " # space at the end
        echo "%{$fg[violet]%}${vim_mode}$(PR_ERROR)$(PR_ARROW) " # space at the end
    elif [[ "${PROMPT_MODE}" == 1 ]]; then
        # echo "%{$fg[violet]%}${vim_mode}$(PR_DIR 1) $(PR_ERROR)$(PR_ARROW) " # space at the end
        echo "%{$fg[pink]%}${vim_mode}$(PR_ERROR)$(PR_ARROW) " # space at the end
    else
        echo "$fg[magenta]${vim_mode}$(PR_ERROR)$(PR_ARROW) " # space at the end
    fi
}
